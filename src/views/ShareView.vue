<template>
  <div v-if="quotation">
    ...
    <div class="share-badge">
      ...
      <router-link to="/nueva" class="toolbar-btn primary" style="padding:6px 14px;font-size:0.7rem;">
        Crear copia
      </router-link>
    </div>
  </div>

  <div v-else class="empty-state" style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;">
    <div class="empty-state-icon">⚠️</div>
    <h2>Cotización no encontrada</h2>
    <p>El enlace no es válido o la cotización está dañada</p>
    <router-link to="/" class="toolbar-btn primary" style="display:inline-block;padding:10px 24px;">
      Volver al inicio
    </router-link>
  </div>
</template>
